package net.blogjava.welldoer.service;
...
public class Neo4jPagesService<T> {
	...
	public Page<T> findAll(Class<T> clazz, Pageable pageable, Filters filters) {
		Collection data = session.loadAll(clazz, filters, convert(pageable.getSort()), new Pagination(pageable.getPageNumber(), pageable.getPageSize()), 1);
